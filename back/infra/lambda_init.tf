resource "aws_lambda_function" "func_init" {
  depends_on = [null_resource.lambda_init]
  ...
  timeout       = "45"
  memory_size   = "512"
  filename      = "${local.codebase_path}/build/init/index.zip"
...
resource "aws_lambda_permission" "allow_api_init" {
  statement_id  = "AllowAPIgatewayInvokation"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.func_init.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:${var.region}:${var.account_id}:${module.api_gateway_v2.this_apigatewayv2_api_id}/*/*/init"
}